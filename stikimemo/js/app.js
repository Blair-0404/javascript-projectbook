...
// 메모 객체
  let Sticky = {
    ...
    add: function () {
      // 창 크기를 구함
      let win_width = $('#sticky_wrap').width() - 250,
      ...
      let $new_sticky = $('.sticky').last();	// 새로 생성된 메모장 객체
    ...
    save: function (current_memo) {
      let idx = localStorage.length;	// 저장된 글 수
      let txt = current_memo.val();	// 작성중인 글

      // 작성된 글이 있으면 저장
      if (txt !== '') {
        let key = prompt('저장할 파일명?', '');
    ...
    get: function list_storage(current_memo) {
      let key;
      let l = localStorage.length; // 총 저장소 길이
      let del_icon = '<i class="fa fa-trash"></i>';
...
// 저장 버튼
  $('#sticky_wrap').on('click', '.save', function () {
    ...
    let current_memo = $(this).parent().siblings('.txt'); // 글 영역 선택
...
// 읽기 버튼
  $('#sticky_wrap').on('click', '.get', function () {
    ...
    let current_memo = $(this).parents('.top_nav').siblings('.side_nav');
    Sticky.get(current_memo);
  });

// 창닫기 버튼
  $('#sticky_wrap').on('click', '.del', function () {
    let current_memo = $(this).parents('.sticky').remove();	// 메모장 객체 제거
  });
...
// 터치 입력
  ...
  $('#sticky_wrap').on('touchmove', '.top_nav', function (e) {
    let $sticky = $(this).parent();	// 메모장 객체
    let event = e.originalEvent;	// 제이쿼리에서 기존 자바스크립트 이벤트 받을 때 필요
    let touchobj = event.changedTouches[0];	// 터치 이벤트 객체
